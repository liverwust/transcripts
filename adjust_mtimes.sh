...
input_listing_without_extensions=`mktemp`
...
if [ `sort -u "$input_listing_without_extensions" | wc -l` -lt `cat "$input_listing" | wc -l` ]
then
	echo "One or more files have duplicate filenames, excluding the extension" >/dev/stderr
	exit 1
fi
...
seq 1 `cat "$input_listing" | wc -l` | while read i; do
	...
	input_filename_wo_ext=`head -n$i "$input_listing_without_extensions" | tail -n1`
...
rm "$input_listing"
rm "$input_listing_without_extensions"
rm "$output_listing"
